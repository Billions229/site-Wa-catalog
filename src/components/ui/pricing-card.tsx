...
function PlanName({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "[&_svg:not([class*='size-'])]:size-4 text-muted-foreground flex items-center gap-2 text-sm font-medium",
        className,
      )}
      {...props}
    />
...
function Badge({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn(
        "text-foreground/80 border-foreground/20 rounded-full border px-2 py-0.5 text-xs",
        className,
      )}
      {...props}
    />
